#!/bin/bash
# Claude Code Dev Stack v2.1 - Installation Verification (Linux/WSL)

echo -e "\n🔍 CLAUDE CODE DEV STACK VERIFICATION"
echo "====================================="

echo -e "\n📦 Agents (Subagents):"
project_agents=$(find .claude/agents -name "*.md" 2>/dev/null | wc -l)
user_agents=$(find ~/.claude/agents -name "*.md" 2>/dev/null | wc -l)
...
echo -e "\n💬 Slash Commands:"
project_commands=$(find .claude/commands -name "*.md" 2>/dev/null | wc -l)
user_commands=$(find ~/.claude/commands -name "*.md" 2>/dev/null | wc -l)
...
if command -v claude &> /dev/null; then
    mcp_count=$(claude mcp list 2>/dev/null | grep -E "playwright|obsidian|brave" | wc -l)
