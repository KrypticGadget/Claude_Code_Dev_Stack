#!/bin/bash
# Master MCP Setup Script for Claude Code Dev Stack (macOS)
...
check_node_version() {
    if ! command -v node &> /dev/null; then
        ...
        echo -e "${GRAY}  Install with: brew install node${NC}"
        return 1
    fi
    ...
    if [ "$(printf '%s\n' "$NODE_MIN_VERSION" "$NODE_VERSION" | sort -V | head -n1)" = "$NODE_MIN_VERSION" ]; then
        ...
        echo -e "${GRAY}  Upgrade with: brew upgrade node${NC}"
        return 1
    fi
}
...
if ! command -v brew &> /dev/null; then
    ...
    echo -e "${GRAY}  Homebrew is recommended for managing packages on macOS${NC}"
    echo -e "${GRAY}  Install with: /bin/bash -c \"\$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)\"${NC}"
fi
...
if [ "$SKIP_PLAYWRIGHT" = false ]; then
    if install_mcp_server "Playwright" "@modelcontextprotocol/server-playwright" ""; then
        ...
        # Install Playwright browsers on macOS
...
if [ -n "$OBSIDIAN_API_KEY" ]; then
    if command -v jq &> /dev/null; then
        ...
        # Fallback without jq (macOS sed requires backup extension)
