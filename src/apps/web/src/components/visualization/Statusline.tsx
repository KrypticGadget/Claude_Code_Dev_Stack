...
import { GitBranch, DollarSign, Cpu, Volume2, Zap, Settings, WifiOff, Wifi, AlertTriangle, Database, Users } from 'lucide-react'
...
interface StatuslineProps {
  ...
  sessions?: { active: number; total: number; status?: string }
}
  ...
  return (
    <div className="statusline">
      ...
      <div className="statusline-row devstack">
        <Link to="/sessions" className="segment sessions">
          <Database size={14} />
          <span className={`value ${getStatusColor(sessions?.status)}`}>
            Sessions: {sessions?.active || 0}/{sessions?.total || 0}
          </span>
          <span className={`status-indicator ${getStatusColor(sessions?.status)}`}>
            {getStatusIcon(sessions?.status)}
          </span>
        </Link>
        ...
        <div className="segment agents">
          ...
          <span className={`value ${getStatusColor(agents.status)}`}>
            Agents: {agents.active}/{agents.total}
          </span>
