...
class PerformanceMonitor:
    ...
    def setup_enhanced_monitoring(self):
        """Setup integration with enhanced monitoring infrastructure"""
        try:
            # Check if enhanced monitoring is available
            monitoring_dir = Path.home() / '.claude' / 'monitoring'
            if monitoring_dir.exists():
                self.enhanced_monitoring_enabled = True
                self.enhanced_monitoring_url = 'http://localhost:8001'
                
                # Create metrics export for enhanced monitoring
                self.export_enhanced_metrics()
            else:
                self.enhanced_monitoring_enabled = False
        except Exception as e:
            print(f"Enhanced monitoring setup error: {e}")
            self.enhanced_monitoring_enabled = False
    
    def export_enhanced_metrics(self):
        """Export metrics to enhanced monitoring format"""
        if not hasattr(self, 'enhanced_monitoring_enabled') or not self.enhanced_monitoring_enabled:
            return
        
        try:
            # Create metrics export file for the enhanced monitoring system
            export_file = Path.home() / '.claude' / 'state' / 'performance_export.json'
            
            export_data = {
                'timestamp': datetime.now().isoformat(),
                'agents': self.metrics.get('agent_metrics', {}),
                'operations': self.metrics.get('operation_metrics', {}),
                'resources': self.metrics.get('resource_metrics', [])[-1:],  # Latest only
                'issues': self.metrics.get('performance_issues', [])[-10:],  # Last 10
            ...
            print(f"Enhanced metrics export error: {e}")
    
    def send_metric_to_enhanced(self, metric_name: str, value: float, labels: dict = None):
        """Send individual metric to enhanced monitoring"""
        if not hasattr(self, 'enhanced_monitoring_enabled') or not self.enhanced_monitoring_enabled:
            return
        ...
        try:
            ...
            requests.post(f"{self.enhanced_monitoring_url}/custom-metric", 
                         json=data, timeout=1)
        except:
            pass  # Don't fail if enhanced monitoring is down
    ...
    def cleanup(self):
        ...
        # Final export to enhanced monitoring
        if hasattr(self, 'enhanced_monitoring_enabled') and self.enhanced_monitoring_enabled:
            self.export_enhanced_metrics()
