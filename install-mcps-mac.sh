...
if command -v node >/dev/null 2>&1 && command -v npm >/dev/null 2>&1; then
    ...
    echo -e "  \033[33mInstall with Homebrew: brew install node\033[0m"
    echo -e "  \033[33mOr download from: https://nodejs.org/\033[0m"
    exit 1
fi
...
if [[ $(uname -m) == "arm64" ]]; then
    echo ""
    echo -e "\033[33m⚠️  Apple Silicon detected\033[0m"
    echo -e "  Some MCPs may require Rosetta 2 for compatibility"
    echo -e "  Install with: softwareupdate --install-rosetta --agree-to-license"
fi
